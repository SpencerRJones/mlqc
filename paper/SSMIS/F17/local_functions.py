...
def read_ssmis_l1c(file):
    ...
    with xr.open_dataset(file, group='S1', decode_timedelta=False) as f:
        ...
        Tbs[:,:,2]   = f.Tc[:,:,2].values #23V


    with xr.open_dataset(file, group='S2', decode_timedelta=False) as f:
        qual[:,:,1] = f.Quality.values
        Tbs[:,:,3:5] = f.Tc.values #37V and H


    with xr.open_dataset(file, group='S3', decode_timedelta=False) as f:
        qual[:,:,2] = f.Quality[:,::2].values
        Tbs[:,:,9]  = f.Tc[:,0::2,0].values #150H
        Tbs[:,:,10] = f.Tc[:,1::2,0].values #150H
        Tbs[:,:,11] = f.Tc[:,0::2,1].values #183 +- 1H
        Tbs[:,:,12] = f.Tc[:,1::2,1].values #183 +- 1H
        Tbs[:,:,13] = f.Tc[:,0::2,2].values #183 +- 3H
        Tbs[:,:,14] = f.Tc[:,1::2,2].values #183 +- 3H
        Tbs[:,:,15] = f.Tc[:,0::2,3].values #183 +- 6H
        Tbs[:,:,16] = f.Tc[:,1::2,3].values #183 +- 6H


    with xr.open_dataset(file, group='S4', decode_timedelta=False) as f:
        qual[:,:,3] = f.Quality[:,::2].values
        Tbs[:,:,5] = f.Tc[:,0::2,0].values #91V
        Tbs[:,:,6] = f.Tc[:,1::2,0].values #91V
        Tbs[:,:,7] = f.Tc[:,0::2,1].values #91H
        Tbs[:,:,8] = f.Tc[:,1::2,1].values #91H
