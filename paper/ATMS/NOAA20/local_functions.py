...
def read_atms_l1c(file):
    ...
    #eia  = np.zeros([nscans,npixs,qual_dim], dtype=np.float32)

    with xr.open_dataset(file, group='S1', decode_timedelta=False) as f:
        qual[:,:,0] = f.Quality.values
        Tbs[:,:,0]  = f.Tc[:,:,0].values #23.8 QV
        #eia[:,:,0]  = f.incidenceAngle.values[:,:,0]

    with xr.open_dataset(file, group='S2', decode_timedelta=False) as f:
        qual[:,:,1] = f.Quality.values
        Tbs[:,:,1]  = f.Tc[:,:,0].values #31.4 QV
        #eia[:,:,1]  = f.incidenceAngle.values[:,:,0]

    with xr.open_dataset(file, group='S3', decode_timedelta=False) as f:
        qual[:,:,2] = f.Quality.values
        Tbs[:,:,2]  = f.Tc[:,:,0].values #88.2 QV
        #eia[:,:,2]  = f.incidenceAngle.values[:,:,0]

    with xr.open_dataset(file, group='S4', decode_timedelta=False) as f:
        qual[:,:,3] = f.Quality.values
        Tbs[:,:,3]  = f.Tc[:,:,0].values #165.5 QH
        Tbs[:,:,4]  = f.Tc[:,:,1].values #183.31+-7 QH
        Tbs[:,:,5]  = f.Tc[:,:,2].values #183.31+-4.5 QH
        Tbs[:,:,6]  = f.Tc[:,:,3].values #183.31+-3 QH
        Tbs[:,:,7]  = f.Tc[:,:,4].values #183.31+-1.8 QH
        Tbs[:,:,8]  = f.Tc[:,:,5].values #183.31+-1 QH
        #eia[:,:,3]  = f.incidenceAngle.values[:,:,0]
    ...
    #Create scan position array
    scanpos = np.zeros([nscans,npixs], dtype=np.int32)
    scanpos[:,0:npixs//2] = np.arange(npixs//2-1,-1,-1)
    scanpos[:,npixs//2:]  = np.arange(0, npixs//2)
...
def attach_gpm_sfctype(lats, lons, time, sensor, gpmversion=None, keepdims=None, ncpus=8):
    ...
    sensor_list = ['GMI', 'SSMI', 'SSMIS', 'ATMS']
