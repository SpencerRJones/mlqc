...
def load_model_tree():
    ...
    for ichan in feature_names:
        ...
        model_file = glob.glob(f'{model_dir}/{sensor}_{satellite}_channel_predictor_{ichan}_ocean_no10.pt')[0]
    ...
    for ichan in feature_names:
        model_file = glob.glob(f'{model_dir}/{sensor}_{satellite}_channel_predictor_{ichan}_nonocean_no10.pt')[0]
...
def run_predictions(tbs, sfccodes, model_tree, keep_dims=True):
    ...
    for ichan, channel in enumerate(features):
        ...
        ocean_model = model_tree[f'{sens}_{sat}_channel_predictor_{channel}_ocean_no10']
        nonocean_model = model_tree[f'{sens}_{sat}_channel_predictor_{channel}_nonocean_no10']
